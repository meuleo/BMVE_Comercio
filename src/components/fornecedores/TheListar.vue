        ...
        <div class="col-lg-12 d-flex align-items-stretch">
          <div class="card w-100" style="background: rgba(255,255,255,0.96);">
              <div class="p-4">
                <div class="row">
                    <div class="col-6 mb-3">
                        <router-link to="/fornecedores/adicionar" class="btn btn-primary btn-sm">
                            Novo +
                        </router-link>
                    </div>
            ...
            <div class="card-body p-4" style="height: 58vh; overflow: auto; ">
              <div class="table-responsive" v-if="$store.state.fornecedores.length > 0">
                <table class="table text-nowrap mb-0 align-middle">
                  <thead class="text-dark fs-4">
                    <tr>
                      ...
                      <th class="border-bottom-0">
                        <h6 class="fw-semibold mb-0">NIF</h6>
                      </th>
                      <th class="border-bottom-0">
                        <h6 class="fw-semibold mb-0">Contacto</h6>
                      </th>
                      <th class="border-bottom-0">
                        <h6 class="fw-semibold mb-0">Localização</h6>
                      </th>
                      <th class="border-bottom-0">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in $store.state.fornecedores.slice().reverse()" :key="item.id">
                      ...
                      <td class="border-bottom-0">
                        <h6 class="fw-semibold mb-1">{{ item.localizacao || 'N/A' }}</h6>
                      </td>
                      <td class="border-bottom-0 text-center">
                        <span class="fw-normal btn btn-sm">Detalhar</span>
                        <span class="fw-normal mx-1 btn btn-sm">
                          <router-link :to="`/fornecedores/editar/${item.id}`">
                            Editar
                          </router-link>
                        </span>
...
export default {
  ...
  methods: {
    eliminar(index, id) {
      ...
      api.eliminar(id, "Fornecedor")
      this.$store.state.fornecedores.splice(index, index);
    }

  },

  async created() {
    // this.$store.state.fornecedores = dadosCarregados();
    await api.iniciarBaseDados();

    this.$store.state.fornecedores = await api.listar("Fornecedor")
